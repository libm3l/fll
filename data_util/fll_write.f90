...
!
CONTAINS
  FUNCTION FLL_WRITE(PNODE,FILE,IOUNIT,FMT,FPAR, ERRMSG,DIAGMESSG) RESULT(OK)
!
...
!
    IF(PRESENT(DIAGMESSG))THEN 
      CALL FLL_WRITE_LIST(PNODE,IOUNIT,FMT_LOC,FPAR,LOC_ERRMSG=LOC_ERRMSG,&
          DIAGMESSG=DIAGMESSG)
    ELSE
      CALL FLL_WRITE_LIST(PNODE,IOUNIT,FMT_LOC,FPAR,LOC_ERRMSG=LOC_ERRMSG)
    END IF
  ...
  END FUNCTION FLL_WRITE
   ...
   SUBROUTINE FLL_WRITE_LIST(PNODE,IOUNIT,FMT,FPAR,LOC_ERRMSG,DIAGMESSG)
!
...
!
   ...
   CHARACTER(LEN=*), OPTIONAL :: DIAGMESSG
!
! Local declarations
!   
   INTEGER(LINT) :: POS
