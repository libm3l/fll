...
!
CONTAINS
   RECURSIVE FUNCTION FLL_NNODES(PNODE,NAME,LTYPE,DATADIM,RECURSE,FPAR,&
         ERRMSG,DIAGMESSG) RESULT(NUMBER)
!
...
!
     IF(RECURSE .AND. ASSOCIATED(PCURR%PCHILD))THEN
       PCHLD => PCURR%PCHILD
       NUMBER = NUMBER + FLL_NNODES(PCHLD,NAME,LTYPE,DATADIM,RECURSE,FPAR,LOC_ERRMSG)
