...
!
   IF(ASSOCIATED(PCHILD))THEN
     PNEW => FLL_MK(PNODE%LNAME,'DIR',0_LINT,0_LINT,FPAR,LOC_ERRMSG)
...
!  NODE IS A FILE NODE
!
    PNEW => FLL_MK(PNODE%LNAME,PNODE%LTYPE,PNODE%NDIM,PNODE%NSIZE,FPAR,ERRMSG=LOC_ERRMSG)
    IF(.NOT.ASSOCIATED(PNEW))THEN
      WRITE(FPAR%MESG,'(A)')' DUPLICATE - error allocating PNEW '
      CALL FLL_OUT(LOC_ERRMSG,FPAR)
      IF(PRESENT(DIAGMESSG))THEN
      ...
      RETURN
    END IF
    CALL FLL_COPY_NODE_ARRAYS(PNODE, PNEW,FPAR,LOC_ERRMSG)
...
!
  RECURSIVE SUBROUTINE FLL_DUPLICATE_RECURSIVE_NODE(PNODE,PDUPL,FPAR,LOC_ERRMSG)
!
...
!
    DO WHILE(ASSOCIATED(PCURR))
     ...
     PNEW => FLL_MK(PCURR%LNAME,PCURR%LTYPE,PCURR%NDIM,PCURR%NSIZE,FPAR,ERRMSG=LOC_ERRMSG)
     IF(.NOT.ASSOCIATED(PNEW))THEN
      WRITE(FPAR%MESG,'(A)')' DUPLICATE - error allocating PNEW '
      CALL FLL_OUT(LOC_ERRMSG,FPAR)
      FPAR%SUCCESS = .FALSE.
     ...
     IF(.NOT.ASSOCIATED(PCHILD))THEN
       CALL FLL_COPY_NODE_ARRAYS(PCURR, PNEW, FPAR,LOC_ERRMSG)
...
!  ADD TO PDUPL LIST
!
     OK = FLL_MV(PNEW,PDUPL,FPAR,ERRMSG=LOC_ERRMSG)
...
! 
!
  SUBROUTINE FLL_COPY_NODE_ARRAYS(PNODE,PNEW,FPAR,LOC_ERRMSG)
!
