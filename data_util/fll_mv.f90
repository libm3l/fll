...
!
CONTAINS 
   FUNCTION FLL_MV(PWHAT,PWHERE,FPAR,ERRMSG,DIAGMESSG) RESULT(OK)
!
...
!
       ...
       IF(PRESENT(DIAGMESSG))THEN
         PSOURCETMP => FLL_MVCP(PWHAT,PWHERE,'M',FPAR,LOC_ERRMSG=LOC_ERRMSG,&
          DIAGMESSG=DIAGMESSG) 
         OK = FPAR%SUCCESS
      ELSE
         PSOURCETMP => FLL_MVCP(PWHAT,PWHERE,'M',FPAR,LOC_ERRMSG=LOC_ERRMSG) 
   ...
   FUNCTION  FLL_MVCP(PWHAT,PWHERE,MODE,FPAR,LOC_ERRMSG, DIAGMESSG) RESULT(PSOURCETMP)
!
